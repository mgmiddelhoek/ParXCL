...
#define ERRORA(f, s)                                                           \
    {                                                                          \
        printf("%s:%d: error: ", prx_filename, prx_lineno);                    \
        printf(f, s);                                                          \
        printf("\n");                                                          \
        bError |= 2;                                                           \
        return 0;                                                              \
...
#define WARNINGA(f, s)                                                         \
    {                                                                          \
        printf("%s:%d: warning: ", prx_filename, prx_lineno);                  \
        printf(f, s);                                                          \
        printf("\n");                                                          \
        bError |= 1;                                                           \
    }
...
#define WRITEM(i)                                                              \
    {                                                                          \
        sh = (i);                                                              \
        if (!fwrite((void *)&sh, sizeof(sh), 1, oFile)) {                      \
            write_error();                                                     \
            return 0;                                                          \
        }                                                                      \
    }
...
static const struct {
    char *name;
    OPR opr;
} FunSt[] = {{"sin", SIN},   {"cos", COS},   {"tan", TAN}, {"asin", ASIN},
             {"acos", ACOS}, {"atan", ATAN}, {"exp", EXP}, {"log", LOG},
             {"log10", LG},  {"sqrt", SQRT}, {"abs", ABS}, {"sign", SGN},
             {"not", NOT}};
...
int prx_error() { return bError; }

/* ========================================================================== */

/* on write error */
int write_error() { ERROR("Error writing output file"); }
    ...
    do {
        ...
        if (*pe == '=') {
            ...
            if (typ == PAR || typ == VAR || typ == AUX) {
                if (typ != PAR && nVals > 3) {
                    WARNINGA("Maximum quantity of values exceeded for %s",
                             name);
                }

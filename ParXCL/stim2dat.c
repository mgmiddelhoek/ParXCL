...
colhead_list makeheader(stimtemplate_list st, modeltemplate mt) {
    ...
    for (s = st, s_max = stimtemplateNIL, nint_max = 0; s != stimtemplateNIL;
         s = s->next) {
    ...
    if (s_max != stimtemplateNIL) /* sweep in first column */
        header = append_colhead_list(
            header, new_colhead(new_tmstring(s_max->name), SWEEP));
    else
        return (header);
    ...
    for (s = st; s != stimtemplateNIL; s = s->next) {
        ...
        if (xext != xspecNIL) { /* stimulus found */
            header = append_colhead_list(
                header, new_colhead(new_tmstring(xext->name), STIM));
    ...
    for (xext = mt->xext; xext != xspecNIL; xext = xext->next) {
        ...
        if (s == stimtemplateNIL) { /* stimulus not found */
            header = append_colhead_list(
                header, new_colhead(new_tmstring(xext->name), UNKN));
...
boolean test_setup(stimtemplate st) {
    ...
    case SLOG:
    case SLN:
        if (st->lval <= 0.0 || st->uval <= 0.0)
            return (FALSE);
        break;
    case ALOG:
    case ALN:
        if ((SIGN(st->lval) != SIGN(st->uval)) || st->lval == 0.0 ||
            st->uval == 0.0)
            return (FALSE);
        break;
    default:
...
datarow_list makedata(colhead_list header, stimtemplate_list st) {
    ...
    for (i = 1; i <= np; i++) {
        data = append_datarow_list(
            data, new_datarow(0, 1, i, new_fnum_list(), new_fnum_list()));
    }

...
void list_parxsymbol(parxsymbol_list sl) {
    ...
    for (; sl != parxsymbolNIL; sl = sl->next) {
        ...
        switch (tag_dbnode(n)) {
            case TAGModel: fputs("Model\n", output_stream);
                break;
            case TAGSystem: fputs("System\n", output_stream);
                break;
            case TAGDatatable: fputs("Datatable\n", output_stream);
                break;
            case TAGStimulus: fputs("Stimulus\n", output_stream);
                break;
            case TAGMeasurement: fputs("Measurement\n", output_stream);
                break;
            default: return;
        }
    }
...
void list_type(tags_dbnode tag) {
    ...
    switch (tag) {
        case TAGModel: fputs("Models:\n", output_stream);
            break;
        case TAGSystem: fputs("Systems:\n", output_stream);
            break;
        case TAGDatatable: fputs("Datatables:\n", output_stream);
            break;
        case TAGStimulus: fputs("Stimuli:\n", output_stream);
            break;
        case TAGMeasurement: fputs("Measurements:\n", output_stream);
            break;
        default: break;
    }
...
static void show_data(dbnode n) {
    ...
    for (h = dt->header; h != colheadNIL; h = h->next) {
        ...
        switch (h->type) {
            case UNKN: fputs("unkn\n", output_stream);
                break;
            case MEAS: fputs("meas\n", output_stream);
                break;
            case CALC: fputs("calc\n", output_stream);
                break;
            case FACT: fputs("fact\n", output_stream);
                break;
            case STIM: fputs("stim\n", output_stream);
                break;
            case SWEEP: fputs("sweep\n", output_stream);
                break;
            default: putc('\n', output_stream);
                break;
        }
...
static void show_stim(dbnode n) {
    ...
    for (; st != stimtemplateNIL; st = st->next) {
        ...
        switch (st->scale) {
            case SLIN: fputs("lin ", output_stream);
                break;
            case SLOG: fputs("log ", output_stream);
                break;
            case SLN: fputs("ln  ", output_stream);
                break;
            case ALIN: fputs("alin", output_stream);
                break;
            case ALOG: fputs("alog", output_stream);
                break;
            case ALN: fputs("aln ", output_stream);
                break;
            default: break;
        }
        fputc('\n', output_stream);
    ...
    for (; sl != parxsymbolNIL; sl = sl->next) {
        ...
        switch (tag_dbnode(n)) {
            case TAGModel: show_mod(n);
                break;
            case TAGSystem: show_sys(n);
                break;
            case TAGDatatable: show_data(n);
                break;
            case TAGStimulus: show_stim(n);
                break;

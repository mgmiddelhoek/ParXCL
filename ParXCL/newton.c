...
static fnum optstep(fnum releps, fnum abseps, fnum fcurr, fnum ffull,
                    inum itmax);
...
inum newton_raphson(vector xl,     /* variable vector */
    ...
    if (trace >= 1) {
        fprintf(trace_stream,
                "iteration: %ld, full step: %ld, partial step: %ld\n",
...
fnum optstep(fnum releps, fnum abseps, fnum fcurr, fnum ffull, inum itmax) {
    ...
    if ((found == FALSE) && (xr >= abseps)) {
        ...
        found =
            brent(xl, xc, xr, fvec_norm, releps, abseps, &itmax, &xmin, &fmin);
    ...
    if (trace >= 3) {
        fprintf(trace_stream, "end directional search, step size : %14.6e\n",
                xmin);
    }

...
numblock make_numblock(modeltemplate mt, systemtemplate st, datatemplate dt,
                       inum trace) {
    numblock numb;
    ...
    if (modex == FALSE) { /* Compiled model */
        ...
        mrs = new_modres(nres, (inum)xstat->sz, naux, (inum)pstat->sz,
                         (inum)cval->sz, (inum)fval->sz, statevectorNIL,
                         statevectorNIL, procedureNIL, procedureNIL,
                         procedureNIL, procedureNIL, procedureNIL);
    ...
    if (rc == FALSE) {
        ...
        modi = new_moddat(m_x, m_a, m_p, m_c, m_f, FALSE, m_r, FALSE, xf, m_jx,
                          m_ja, FALSE, pf, m_jp);
        ...
        if ((parmset == pset_listNIL) || (xextgrp == xgroup_listNIL) ||
            (rci == FALSE)) {
...
void get_xst(modeltemplate mt, datatemplate dt, fnum_list xval,
    ...
    for (xs = mt->xext; xs != xspecNIL; xs = xs->next) {
        ...
        for (header = dt->header, index = 0; header != colheadNIL;
             header = header->next, index++) {
            if (strcmp(xs->name, header->name) == 0) {
                break;
            }
        }
        ...
        case UNKN:
            ...
            if (trace >= 1) {
                fprintf(trace_stream, "%-3ld: %3ld   -  (U): %-8s ", (long)i++,
                        (long)idxu++, xs->name);
...
void get_pst(modeltemplate mt, systemtemplate st, stateflag_list pstat,
    ...
    for (ps = mt->parm; ps != pspecNIL; ps = ps->next) {
        ...
        case TAGPmeas:
            ...
            if (trace >= 1) {
                fprintf(trace_stream, "%-3ld:   - (M): %-8s ", (long)i++,
                        ps->name);
        ...
        case TAGPcalc:
            ...
            if (trace >= 1) {
                fprintf(trace_stream, "%-3ld:   - (C): %-8s ", (long)i++,
                        ps->name);
        ...
        case TAGPfact:
            ...
            if (trace >= 1) {
                fprintf(trace_stream, "%-3ld:   - (F): %-8s ", (long)i++,
                        ps->name);
                fprintf(trace_stream, "= %.8e\n", to_Pfact(pv)->factval);
            }
            break;
...
inum set_xst(modeltemplate mt, datatemplate dt, stateflag_list xstat,
             inum_list xtrans) {
    xspec xs;
    ...
    if (header == colheadNIL) { /* add header */
        dt->header = append_colhead_list(
            dt->header, new_colhead(new_tmstring(RESNORM), FACT));
        return (1);
    } else {
        header->type = FACT;
        return (0);
...
void read_numblock_x(modres mrs, xgroup_list xg, modeltemplate mt,
                     datatemplate dt) {
    colhead_list header;
    inum_list xtrans;
    ...
    for (header = dt->header, residx = 0; header != colheadNIL;
         header = header->next, residx++) {
        if (strcmp(RESNORM, header->name) == 0) {
            break;
        }
    }

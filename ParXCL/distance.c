...
static boolean dist_step_direction(vector dist, vector aux, vector lagrange,
                                   vector lambda, vector d_lambda, vector c_res,
                                   matrix jx, matrix ja, inum trace);

static fnum dist_step_size(vector dist, vector aux, vector lambda, vector c_res,
                           matrix jx, matrix ja, fnum min_alpha, inum trace);

/***********************************************************************/

/* Allocate global data structures */

void new_distance(inum nl,    /* number of Lagrange multipliers */
...
boolean distance(vector dist,     /* distance vector */
    ...
    for (iter = 0; (conv == FALSE) && (iter <= maxiter); iter++) {
        ...
        sd = dist_step_direction(dist, aux, lagrange, lambda, d_lambda, c_res,
                                 jx, ja, trace - 1);
        ...
        for (i = 0; i < VECN(powmu); i++) {
            if (iter == 0) {
                ...
                VEC(powmu, i) =
                    MAX(fabs(VEC(lambda, i)),
                        0.5 * fabs(VEC(powmu, i) + fabs(VEC(lambda, i))));
            }
        }
...
boolean
dist_step_direction(vector dist,     /* current distance vector */
    ...
    if (solvesym_m(jjt, jdf, jdf) == FALSE) {
        ...
        if (trace >= 1) {
            fputs(
                "Jacobian decomposition failed: indefinite model constraints\n",
                trace_stream);
        }
        return (FALSE);
    }
...
static fnum constr_linef(fnum alpha) {
    ...
    b = ext_constraints(n_dist, n_aux, TRUE, c_res, FALSE, matrixNIL, matrixNIL,
                        g_trace - 1);
...
fnum dist_step_size(vector dist,    /* current distance */

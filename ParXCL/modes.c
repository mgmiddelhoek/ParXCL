...
boolean modes(inum neq,     /* maximum number of equations */
    ...
    for (iter = 1, loc_iter = 1, modify = TRUE;
        ...
        b = objective(p, rf, &res, jf, &jacp, modify, FALSE, &npoints, &meval_f,
                      &meval_jx, &meval_jp, trace - 4);
        ...
        b = step_direction(res, jacp, dp, &dc, s_val, s_vec, stol, p0, &rank,
                           trace - 2);
        ...
        if (conv == TRUE) { /* can't improve objective by dp */
            ...
            prox =
                proximity(res, s_val, ng, rank, opt, tol, &maxcon, trace - 1);
            ...
            if (prox == FALSE) { /* not good enough */
                ...
                b = modify_point_set(res, ng, s_val, s_vec, jacp, rank, dp, &dc,
                                     &res_norm, &npoints, p0, wrkv, wrkm,
                                     trace - 1);
        ...
        if (conv == FALSE) { /* determine the optimal step size */
            ...
            alpha =
                step_size(res_norm, rtol, min_alpha, bound_alpha, trace - 2);
    ...
    if (trace >= 1) { /* report effort */
        ...
        if (cpu_time > -1) {
            fprintf(trace_stream, "CPU time: %.0f sec\n\n",
...
boolean step_direction(vector res,  /* current residual vector */
    ...
    *rank = svd(jacp, jacp, s_val, s_vec, stol);
    ...
    if (trace >= 2) {
        ...
        print_matrix(pst, s_vec);
        tm_endprint(pst);
        fprintf(trace_stream, "Estimated rank = %ld, Condition number = %.*e\n",
...
void check_bounds(vector p,    /* scaled parameter vector */
...
boolean
eval_obj_line(fnum alpha, /* step size */
    ...
    lf = objective(p0, rf, &res, jf, &jacp, FALSE, FALSE, &npoints, &meval_f,
                   &meval_jx, &meval_jp, ls_trace - 1);
...
fnum step_size(fnum res_norm,      /* current minimum for step = 0 */
    ...
    if (trace >= 1) {
        fprintf(trace_stream, "Directional search, step size : %.*e\n",
                FNUM_DIG, xmin);
    }
...
void conf_lim(vector p,     /* scaled parameter values */

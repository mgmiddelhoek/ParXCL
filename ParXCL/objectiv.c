    ...
    for (grp = 0; grp <= (all == TRUE ? 2 : 0); grp++) {
        ...
        for (xi = 0, i = 0; xs != xsetNIL;) {
            ...
            *(xsindex + xi) = xs; /* setup xsindex to point */
            ...
            ok = residual(xs, p, rf, subr, jf, subj, sf, s,
                          &lmc_r, &lmc_jx, &lmc_jp, trace - 2);
    ...
    *mc_jp += lmc_jp;
    ...
    if (trace >= 1) {
        ...
        fprintf(trace_stream,
                "model evaluations: f %ld, Jx %ld, Jp %ld\n\n",
...
inum remove_data_point(
                       inum n, /* xsindex of point to be moved */
                       inum g, /* target group */
                       inum trace /* trace level */
) {
    TMPRINTSTATE *pst;
